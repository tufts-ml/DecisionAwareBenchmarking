...
- Tasks: KS = Knapsack (synth), KS-E = Knapsack (energy), En = Energy scheduling, BA = Budget allocation, Cu = Cubic Top-K, BM = Bipartite matching, Pf = Portfolio, AS = Aerial survey (asurv), CC = Cook County, SH = Speed humps, SP-s = Shortest path (synth), SP-p = Shortest path (planted), PG-ms = PG misspec, SP-W = Warcraft shortest path.
