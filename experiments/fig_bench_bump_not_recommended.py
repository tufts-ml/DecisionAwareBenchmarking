"""
fig_bench_bump_not_recommended.py
---------------------------------
Bump charts for the task subset NOT recommended in the paper:
  knapsack (gen), knapsack (real), energy, cubic, bipartite matching,
  shortestpath (warcraft 12x12)
plus an average-rank column.
...
if NAME_SUFFIX:
    print(f"[data] {DATA_JSON}  ->  output suffix '{NAME_SUFFIX}'")
...
def draw(with_ci=False):
    ...
    fname = f"fig_bench_bump_not_recommended_{tag}{NAME_SUFFIX}"
